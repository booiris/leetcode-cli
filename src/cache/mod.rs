...
impl Cache {
    ...
    pub async fn download_problems(self) -> Result<Vec<Problem>, Error> {
        ...
        for i in &self.0.conf.sys.categories.to_owned() {
            let json = self
                ...
                .await?;
            parser::problem(&mut ps, json, i.clone()).ok_or(Error::NoneError)?;
        }
    ...
    /// Get daily problem
    pub async fn get_daily_problem_id(&self) -> Result<String, Error> {
        parser::daily(
            self.clone()
                .0
                .get_question_daily()
    ...
    pub async fn get_question(&self, rfid: String) -> Result<Question, Error> {
